...
def get_file_path(filename):
    # Check if we are running in a serverless environment (Google Cloud Functions or Cloud Run)
    is_serverless = os.getenv("K_SERVICE") or os.getenv("FUNCTION_TARGET")
    if is_serverless:
        tmp_path = os.path.join('/tmp', filename)
        if not os.path.exists(tmp_path):
            src_path = os.path.join(app.root_path, filename)
            if os.path.exists(src_path):
                import shutil
                try:
                    shutil.copy2(src_path, tmp_path)
                except Exception as e:
                    print(f"Error copying seed file {filename} to /tmp: {e}")
        return tmp_path
    return os.path.join(app.root_path, filename)

# In-memory database caches to prevent repetitive disk I/O
_issues_cache = None
_crowd_cache = None
...
def load_issues():
    ...
    issues_path = get_file_path('issues.json')
...
def load_crowd_data():
    global _crowd_cache
    if _crowd_cache is not None:
        return _crowd_cache
        
    crowd_path = get_file_path('mock_crowd_data.json')
...
# Security headers middleware to guard against clickjacking, XSS, and MIME-sniffing
@app.after_request
